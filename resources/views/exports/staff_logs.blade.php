@php
    ...
    $minStartDate = $staffLogs->min('created_at');
    $maxEndDate = $staffLogs->max('created_at');
...
<table>
    ...
    @foreach($staffLogs as $log)
        <tr>
            ...
            <td style="border: 1px solid #101010; text-align: left;">{{ $log->staff->name }}</td>
            <td style="border: 1px solid #101010; text-align: left;">{{ $log->action }}</td>
            <td style="border: 1px solid #101010; text-align: left;">{{ \Carbon\Carbon::parse($log->dateTime)->format('F j, Y h:i A') }}</td>
        </tr>
    @endforeach
    </tbody>
</table>
